pipeline {
    ...
    environment {
    DOCKERHUB_CREDENTIALS = credentials('brianmba-dockerhub')
    APP_NAME = "brianmba/bluegreen-rollout-oct"
    }
    stages { 
        stage('SCM Checkout') {
            steps{
           git branch: 'main', url: 'https://github.com/breezylouie/Argo-rollout-nodejs.git'
            }
        }
        // run sonarqube test
        stage('Run Sonarqube') {
